...
ag_commod %>% bind_rows(an_commod) %>%
  mutate(price2010=gdp_deflator(2010,1975)*price, #price per kg
...
all_traded %>%
  separate(subsector,sep = " ", into=c('first','second','third','fourth'))%>%
...
all_traded %>%
  separate(subsector,sep = " ", into=c('first','second','third','fourth','fifth'),extra="merge") %>%
  filter(!is.na(fifth)) %>%
...
ag_an_value %>% select(scenario,region,GCAM_commodity,year,price2010) %>%
  ...
  mutate(totval=totalprod*price2010,exportval=exported*price2010)->exports_commod

exports_commod %>%
  group_by(scenario,region,year) %>%
  summarize(exportval=sum(exportval,na.rm=T),totalval=sum(totval,na.rm=T)) %>%
  mutate(value=exportval/totalval, variable="Export_pct", Unit="%") %>%
...
agval_region %>%
  ...
  mutate(value=if_else(is.na(totagemploy_pred),NA_real_,value/(totagemploy_pred)), variable="AgLabProd",
         Unit="$ per ag. worker") %>%
